...
function scserver(command, context, properties) {
    if (command.scserver) {
        ...
        if (cmds.schedLoop) {
            ...
            if (!context.oscSched) {
                var sendFn = function (time, packets) { return context.scserver.send.bundle(time, packets); };
                context.oscSched = new OSCSched_1.default(sendFn);
            }
...
function resolveFuncs(command, context, properties) {
    return lodash_1.default.mapValues(command, function (value) { return _callIfFn(value, context, properties); });
}
...
function _callIfFn(thing, context, properties) {
    return lodash_1.default.isFunction(thing) ? thing(context, properties) : thing;
}
