...
var utils_1 = require("../../server/osc/utils");
/**
...
var OSCSched = /** @class */ (function () {
     ...
     */
    function OSCSched(sendFn, latency, setTimeoutFn, clearTimeoutFn) {
        if (latency === void 0) { latency = 0.05; }
        if (setTimeoutFn === void 0) { setTimeoutFn = setTimeout; }
        if (clearTimeoutFn === void 0) { clearTimeoutFn = clearTimeout; }
        this.sendFn = sendFn;
        this.latency = latency;
        this.setTimeout = setTimeoutFn;
        this.clearTimeout = clearTimeoutFn;
        this.getNextFn = function (now, memo) { return undefined; };
        this.epoch = lodash_1.default.now();
     ...
     */
    OSCSched.prototype.schedLoop = function (getNextFn, epoch) {
    ...
    OSCSched.prototype._schedNext = function (memo, logicalNow) {
        if (this.timerId) {
            this.clearTimeout(this.timerId);
            this.timerId = undefined;
        }
        var now = (lodash_1.default.now() - this.epoch) / 1000;
        if (!logicalNow) {
            logicalNow = now;
        }
        var next = this.getNextFn(logicalNow, memo || { i: 0 });
     ...
     */
    OSCSched.prototype._jitSend = function (now, delta, next) {
        var _this = this;
     ...
     * @param  {object} event
     */
    OSCSched.prototype._send = function (event) {
        this.sendFn(utils_1.deltaTimeTag(event.time, this.epoch), event.msgs);
    };
    return OSCSched;
}());
