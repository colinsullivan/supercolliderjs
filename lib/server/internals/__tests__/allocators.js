...
var alloc = tslib_1.__importStar(require("../allocators"));
// doesnt work for jest:
// import * as alloc from '../allocators';
describe("inc", function () {
    it("should inc from undefined (default = 0) to 1", function () {
        var _a = alloc.increment(0), n = _a[0], s = _a[1];
        expect(n).toEqual(1);
        expect(s).toEqual(1);
    });
    it("should inc from 1 to 2", function () {
        var _a = alloc.increment(1), n = _a[0], s = _a[1];
        expect(n).toEqual(2);
        expect(s).toEqual(2);
    });
});
describe("block allocator", function () {
    ...
    describe("alloc", function () {
        it("should alloc a 2 chan", function () {
            var _a = alloc.allocBlock(is, 2), addr = _a[0], state = _a[1];
            expect(addr).toBe(0);
            var fb = alloc.freeBlockList(state);
        ...
        it("should alloc a 2 chan, twice", function () {
            var _a;
            var _b = alloc.allocBlock(is, 2), addr = _b[0], state = _b[1];
            _a = alloc.allocBlock(state, 2), addr = _a[0], state = _a[1];
            expect(addr).toBe(2);
            var fb = alloc.freeBlockList(state);
        ...
        it("should throw on allocation failed", function () {
            ...
            expect(function () { return alloc.allocBlock(state, 2); }).toThrow();
        });
        ...
        it("should find free space of 2 after a free space of 4", function () {
            ...
            var _a = alloc.allocBlock(is, 2), b = _a[0], state = _a[1];
            expect(b).toBe(2);
            var fb = alloc.freeBlockList(state);
            expect(fb).toEqual([]);
        });
